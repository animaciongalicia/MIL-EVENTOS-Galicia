...
/eventos-en-galicia                       Hub SEO local por provincia, FUERA del menú principal
  ...
  /eventos-en-galicia/pontevedra          Rías Baixas, Vigo incluido (§11)
...
| `FooterGlobal` | Footer de 5 columnas (`bg-atlantico-800`, **`max-w-6xl`** — el footer es la única excepción al `max-w-5xl` de §12, por ser banda oscura a sangre completa; `grid-cols-2 lg:grid-cols-6`, marca en `col-span-2`): **marca + contacto + dirección** · **Empresas** (las 4 pilares B2B) · **Servicios** (Actividades, Espacios, Celebraciones) · **Mil Eventos** (Blog, Quiénes somos, Contacto) con **Ecosistema** (SUUNIA/Sea/Luxe) apilado justo debajo · **Zonas de Galicia** (hub + 4 provincias, SEO local). Cierra con © + enlaces legales. Los títulos de columna son enlaces a su landing. Sin listar todas las sub-páginas (se probó un footer-mapa completo y quedaba apelotonado). |
...
- **Anti-canibalización blog ↔ pilares (pilar y categoría DESACOPLADOS):** cada post lleva dos campos en el frontmatter — `pilar` (una de las 7 pilares de `PILARES` en `lib/blog.ts`: las 6 corporativas + `celebraciones`, el único no-corporativo, que enlaza al clúster festivo) y `categoria` (editorial). El **`pilar` no es taxonomía navegable**: solo alimenta el enlace interno obligatorio del cierre del post (§9) vía `PILAR_HREF`/`PILAR_LABELS`, así el blog nunca compite por keyword con la pilar. La **`categoria` es la taxonomía navegable** y agrupa por TIPO de contenido, no por servicio: `casos` (casos reales de eventos que montamos — prueba social editorial, §6, no ficha de producto ni catálogo e-commerce §1), `guias`, `ideas`, `errores` (`CATEGORIA_LABELS`). Como no duplican la intención de búsqueda de ninguna pilar, las páginas `/blog/categoria/[categoria]` **sí se indexan** (ya no llevan noindex). Los relacionados (`getPostsRelacionados`) muestran primero la misma categoría editorial y rellenan con los más recientes, para que todo post ofrezca por dónde seguir. Al añadir un post, define SIEMPRE `pilar` + `categoria`.
...
- Posts de blog: un archivo `.mdx` por post en `/content/blog/`, con frontmatter que incluya `pilar` (una de las 7 pilares de `PILARES`: `team-building`, `incentivos`, `jornadas-outdoor`, `congresos-y-convenciones`, `gastronomia-y-vinos`, `espacios-y-recursos` y `celebraciones` — decide el enlace interno del cierre; `celebraciones` es el único que enlaza fuera del sitio corporativo, al clúster festivo) y `categoria` (editorial: `casos`, `guias`, `ideas`, `errores` — la taxonomía navegable; `casos` = eventos reales que hemos montado, como prueba social). Ver §9 para por qué van desacoplados.
...
- ~~Redirecciones desde la web anterior~~ — mapeadas en `next.config.js` → `redirects()` a partir del export de Search Console (Coverage + Performance) y GA4 aportado por el cliente. Cubre las URLs con tráfico/impresiones reales, no el sitemap completo (que incluye ~400 páginas de adjuntos de imagen de WordPress sin valor). **Revisado (sept. 2026):** cruzadas las 161 URLs con tráfico/impresiones de los exports GSC/GA contra las reglas — cobertura 100% (las páginas-adjunto de WP sin regla propia caen en su padre `:path*` o hacen 404 sin valor SEO). Auditoría de enlaces internos: 50 páginas rastreadas, 0 enlaces rotos, todas con title+description.
- **Aviso legal / Política de privacidad:** las páginas existen (`/aviso-legal`, `/politica-de-privacidad`) y ya muestran los **datos identificativos reales** desde `data/contacto.ts` (razón social Inversiones SHISO S.L., CIF B70319923, domicilio Ronda de Montealto 4, 5ºA, 15002 A Coruña). Único dato opcional que falta si el cliente lo quiere añadir: los datos de inscripción en el Registro Mercantil (tomo/folio/hoja) — no es obligatorio para la LSSI pero suma.
...
- **Vigo — SÍ se menciona.** Lo que se descontinuó es solo un **servicio concreto**: *alquilar locales propios en Vigo para cumpleaños* (ya no se ofrece, no era escalable). NO la ciudad. Vigo es la mayor ciudad de Pontevedra y un generador de tráfico real (ver analíticas): la agencia **organiza eventos, team building, cenas y celebraciones en Vigo y las Rías Baixas, y localiza espacios allí** — todo eso se escribe con normalidad. Lo único que no se reconstruye es esa oferta específica de "alquiler de local en Vigo para cumpleaños". La redirección de la URL antigua `/locales-en-vigo/` apunta hoy a localización de espacios (defendible: la agencia sí busca/localiza espacios en Vigo; reevaluable, ver plan).
...
- **Páginas de provincia (SEO local):** clúster `/eventos-en-galicia` (hub + `/a-coruna`, `/pontevedra`, `/lugo`, `/ourense`) para recuperar el long-tail geográfico de la web anterior (`…-en-coruna`, etc.) sin canibalizar las pilares (§9): las pilares compiten por "en Galicia" y estas por "en \[provincia\]". Contenido en `data/provincias.ts` (§10, datos fuera del JSX), tono Rentabilista B2B (§5), sin cifras por provincia inventadas (§6). Cada provincia es una página completa: hero, intro, ciudades/zonas, tipos de evento, razones, dos `CitaDestacada` y un **FAQ propio con JSON-LD `FAQPage`** (SEO/LLM local). Enlazan a las pilares (enlace interno §9) y algunas a las marcas hermanas cuando encaja (Sea Galicia en Pontevedra, SUUNIA en incentivos multi-día). **FUERA del menú principal** — solo desde el hub y la tira "Zonas de Galicia" del `FooterGlobal`. La página de Pontevedra trabaja las Rías Baixas **incluyendo Vigo con normalidad** (§11), sin reconstruir la oferta descontinuada de alquiler de local en Vigo para cumpleaños.
...
- **Logo de marca (real, del cliente):** wordmark "MILEVENTOSGALICIA" — **MIL** y **GALICIA** en naranja de marca `#F4A037`, **EVENTOS** en negro (fondo claro) o blanco (fondo oscuro). Extraído del `.ai` original a PNG transparente. Archivos en `public/images/`: `logo-mileventos.png` (wordmark, versión clara → `Header`), `logo-mileventos-blanco.png` (wordmark, versión blanca → `FooterGlobal`), `logo-mileventos-tagline.png` (con la URL, versión clara → JSON-LD `logo`/`image` del layout). El favicon (`app/icon.tsx`, `app/apple-icon.tsx`) es una "M" naranja sobre negro, eco del lockup oscuro. El **naranja de marca es complementario del acento `atlantico`** (teal), por eso el logo resalta sin chocar; NO se añade el naranja como token de paleta general — es solo del logo/favicon. El nombre "Mil Eventos Galicia" en texto se reserva para `alt`/`aria-label` y metadatos.
- **Contraste deliberado, no un único crema plano:** el `Hero` compartido en modo claro (páginas pilar interiores: team-building, outdoor, traslados…) y los bloques largos de texto van en `cream-100/200` (nunca oscuros — esa fue la corrección inicial). Pero el cierre de página (`CTAFinal`) y el `FooterGlobal` sí son bloques sólidos en `atlantico-700`/`atlantico-800` con texto claro — acentos puntuales y acotados, no un wash de toda la sección.
- **Hero cinematográfico de las páginas reina:** las 4 landings de categoría (`/eventos-empresa`, `/actividades`, `/espacios-y-recursos`, `/celebraciones`) usan el `Hero` con `cinematografico` (foto a sangre completa + degradado `atlantico-800`/`terracota-900` de abajo arriba + texto encima: `eyebrow` + titular + subtítulo + CTA claro), al estilo del hero de la home, `min-h-[52vh]`. Es la excepción deliberada a "el hero interior va en crema": las reinas son casi portadas de sección y mandan visualmente; sus páginas pilar hijas siguen con el hero claro por defecto, un escalón por debajo. El acento (atlántico/terracota) lo marca `variante`.
- **Home (`app/(site)/page.tsx`) — diseño propio, no plantilla:** la home NO usa el componente `Hero` ni `LandingCategoria`; monta su propio **hero cinematográfico** (`min-h-[54vh]` — deliberadamente contenido y elegante, no a pantalla completa; foto de evento full-bleed con degradado `atlantico-800` de abajo arriba, eyebrow + titular serif grande + subtítulo + 2 CTAs invertidos, y las 3 cifras reales integradas en una barra `border-cream-50/20` dentro del propio hero). Debajo, un bloque de **servicios con jerarquía**: un banner destacado ancho para Eventos de empresa (prioridad B2B, §11) y una fila de 3 mosaicos con el texto SOBRE la imagen (degradado + título + flecha), en vez de repetir cuatro tarjetas foto+caja iguales. El resto de secciones alternan fondos `cream-50` / `cream-100` / `cream-200` para dar ritmo, con el hero y el `CTAFinal` como los dos anclas oscuras. Debajo del bloque de servicios, una sección **"Montamos eventos en toda Galicia"** con 4 mosaicos de provincia (foto + nombre) que enlazan a `/eventos-en-galicia/[provincia]` — mete el SEO local y el enlace interno a las provincias también en la home, no solo en el footer. Regla de "menos fotos sueltas, más jerarquía y aire": la home concentra la fotografía en el hero, el mosaico de servicios, el de provincias y el blog — no encadena bloques de foto+texto planos. Cada epígrafe de sección va como eyebrow en `uppercase tracking-[0.2em] text-atlantico-600`. Las tarjetas (`GridEjemplos`, `LandingCategoria`, `TarjetaPost`) van en `cream-50` (blanco) con sombra para despegarse del fondo de página; los bloques "Cómo trabajamos" de las páginas pilar van en una banda `cream-200` para romper la monotonía. Si una página nueva queda de un único tono de principio a fin, algo se ha hecho mal.
